...
export const createPost = async (req, res) => {
    try {
        ...
        for (const file of files) {
            ...
            const cloudResponse = await uploadBase64Image(base64Image, CLOUDINARY_FOLDERS.POST_IMAGES);
            media.push({
                url: cloudResponse.secure_url,
                publicId: cloudResponse.public_id,
                type: "image",
...
export const getAllComments = async (req, res) => {
    try {
        const { id } = req.params; //post id
        const comments = await Comment.find({ post: id }).populate("author", "userName profilePic");
        if (!comments)
            return res.status(400).json({
                success: false,
...
export const toggleBookmarkPost = async (req, res) => {
    try {
        ...
        const isBookmarked = authUser.bookmarks.some((bookmarkId) => bookmarkId.toString() === id);
        if (isBookmarked) {
            //remove bookmark
