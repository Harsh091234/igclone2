    ...
    try {
        ...
        for (const file of files || []) {
            ...
            if (type === "image") {
                ...
                const uploaded = await uploadBase64Image(base64Image, CLOUDINARY_FOLDERS.CHAT_IMAGES);
                media.push({ url: uploaded.secure_url, type });
            }
            //  VIDEO
            else if (type === "video") {
                const uploaded = await uploadVideo(file.buffer, CLOUDINARY_FOLDERS.CHAT_VIDEOS);
                media.push({ url: uploaded.secure_url, type });
            }
            //  PDF / DOC / XLS / AUDIO
            else {
                const uploaded = await uploadRaw(file.buffer, CLOUDINARY_FOLDERS.CHAT_RAW_FILES);
                media.push({ url: uploaded.secure_url, type });
            }
        }
